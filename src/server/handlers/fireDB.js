...
class DB {
  ...
  WriteAccountToFirebase(account) {
    return new Promise((resolve, reject) => {
      this.Con.ref('accounts').push(account.toJSON(true), () => resolve());
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      ref
        ...
        .then((snapshot) => {
					...
					const account = null;
					accounts.forEach((accountSingle, key) => {
						account = accountSingle;
					});
					console.log("found account: ", account);
          if (
            AccountDef.CheckPasswordAgainstHashed(password, account.Password)
          ) {
            resolve(account);
          } else {
