...
class DB {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.getAccountById(id)
        .then(updateableAccount => {
          ...
          return this.Con.ref('accounts/' + id).set(updateableAccountData);
        })
        .then(result => {
      ...
      this.Con.ref(`accounts`)
        ...
        .then(snapshot => {
          ...
          if (!accountRaw) {
            ...
            resolve(new Account(getSingleValueFromSnapshot(accountRaw)));
          }
        })
        .catch(error => {
  ...
  async getAccountsByDivisionRoleQualifies(division, role, qualifies) {
    try {
      const snapshot = await this.Con.ref('accounts').once('value');
      const snapMap = snapshot.val();
      const finalAccounts = [];
      for (let key in snapMap) {
        if (!snapMap.hasOwnProperty(key)) {
          continue;
        }
        let drqs = [`${division}-${role}`, `${division}-${role}-q`];
        if (qualifies && Dancer.IsPreviousDivisionAvailable(division)) {
          drqs.push(`${Dancer.GetPreviousDivision(division)}-${role}-q`);
        }
        if (
          (qualifies === null &&
            snapMap[key].wsdcDancer.division === division &&
            snapMap[key].wsdcDancer.role === role) ||
          (qualifies !== null &&
            drqs.indexOf(snapMap[key].wsdcDancer.divisionRoleQualifies) != -1)
        ) {
          finalAccounts.push(new Account(snapMap[key]));
        }
      }
  ...
  async getDancer(wsdcid) {
    try {
      const snapshot = await this.Con.ref('dancers/' + wsdcid).once('value');
    ...
    return new Promise((resolve, reject) => {
      ...
      ref
        ...
        .then(snapshot => {
          ...
          if (password) {
            if (
              Account.CheckPasswordAgainstHashed(password, account.password)
            ) {
              resolve(key);
            } else {
