...
class Account {
  ...
  Copy(data) {
    for (let key in this) {
      if (!this.hasOwnProperty(key) || !data.hasOwnProperty(key) || !data[key])
        continue;
      this[key] = data[key];
    }
  }
  ...
  static CheckPasswordAgainstHashed(password, hashed) {
      console.log("Checking password: ", password, hashed, bcrypt.hashSync(password, 10));
      return bcrypt.hashSync(password, 10) === hashed;
  }
}
